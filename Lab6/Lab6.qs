...
namespace Lab6 {
    ...
    operation Exercise1 (
        ...
        // setting up input/output qubits
        let len = Length(input);
        use output = Qubit[len];
        use input_copy = Qubit[len];
        for i in 0 .. len - 1 {
            if (input[i]) {
                X(input_copy[i]);
            }
        }
        // feed input/output to operation
        op(input_copy, output);
        // measuring
        mutable measures = [false, size=len];
        for i in 0 .. len - 1 {
            set measures w/= i <- M(output[i]) == Zero ? false | true;
        }
        ResetAll(input_copy);
        ResetAll(output);
        return measures;
    ...
    operation Exercise2 (
        ...
        mutable measures = [false, size=inputSize];
        ...
        for i in 0 .. inputSize - 1 {
            set measures w/= i <- M(input[i]) == Zero ? false | true;
        }
        ResetAll(input);
        ResetAll(output);
        return measures;
